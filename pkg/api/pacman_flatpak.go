...
// Module: pacman_flatpak.go
// Description: Provides functions for managing flatpak apps when using the Pacman package manager.
...
func FlatpakInstall(app string) error {
	...
	if _, err := exec.LookPath("flatpak"); err != nil {
		...
		StatusT("Flatpak is not installed. Installing from pacman repositories...")
		if PackageAvailable("flatpak", "") {
			cmd := exec.Command("sudo", "pacman", "-Syu", "flatpak", "--noconfirm")
	...
	if version < "1.14.4" {
		...
		cmd := exec.Command("sudo", "pacman", "-Syu", "flatpak")
		cmd.Stdout = os.Stdout
		cmd.Stderr = os.Stderr
		if err := cmd.Run(); err != nil {
