...
// Module: preload_daemon_pacman.go
// Description: Background daemon for refreshing app list files when using the Pacman package manager.
...
// This is Pacman-specific and monitors Pacman database changes
func (d *PreloadDaemon) refreshPackageAppStatus() error {
	...
	// Check if Pacman local database has changed since last check
	// Pacman uses /var/lib/pacman/local/ as the local database directory
	pacmanLocalDbPath := "/var/lib/pacman/local"
	pacmanLocalDbStat, err := os.Stat(pacmanLocalDbPath)
	if err != nil {
		logger.Warn(api.Tf("Could not stat Pacman local database: %v\n", err))
		return nil // Not an error if Pacman database doesn't exist
	}

	// Get the modification time
	pacmanDbTime := pacmanLocalDbStat.ModTime()
	timestampFile := filepath.Join(preloadDir, "timestamps-pacman-db")

	// Read the last saved timestamp
	if savedTime, err := os.ReadFile(timestampFile); err == nil {
		savedTimeStr := string(savedTime)
		if savedTimeStr == pacmanDbTime.Format(time.RFC3339) {
			// No changes, skip refresh
			return nil
		}
	}

	// Pacman database has changed, refresh all package app statuses
	logger.Info("Pacman database changed, refreshing package app statuses...")
	...
	if err := os.WriteFile(timestampFile, []byte(pacmanDbTime.Format(time.RFC3339)), 0644); err != nil {
		logger.Warn(api.Tf("failed to save Pacman database timestamp: %v\n", err))
	}
