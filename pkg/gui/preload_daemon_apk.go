...
func (d *PreloadDaemon) refreshPackageAppStatus() error {
	...
	if err != nil {
		logger.Warn(fmt.Sprintf("Could not stat APK database: %v\n", err))
	...
	if err := api.RefreshAllPkgAppStatus(); err != nil {
		logger.Error(fmt.Sprintf("failed to refresh package app statuses: %v\n", err))
	...
	if err := os.WriteFile(timestampFile, []byte(apkDbTime.Format(time.RFC3339)), 0644); err != nil {
		logger.Warn(fmt.Sprintf("failed to save APK database timestamp: %v\n", err))
	}
