...
// Module: preload_daemon_apk.go
// Description: Background daemon for refreshing app list files for APK package manager.
...
// refreshPackageAppStatus refreshes the status of all package-based apps
// This is APK-specific and monitors APK database changes
func (d *PreloadDaemon) refreshPackageAppStatus() error {
	...
	// Check if APK database has changed since last check
	// APK uses /lib/apk/db/installed as the database file
	apkDbPath := "/lib/apk/db/installed"
	apkDbStat, err := os.Stat(apkDbPath)
	if err != nil {
		logger.Warn(api.Tf("Could not stat APK database: %v\n", err))
		return nil // Not an error if APK database doesn't exist
	}

	// Get the modification time
	apkDbTime := apkDbStat.ModTime()
	timestampFile := filepath.Join(preloadDir, "timestamps-apk-db")

	// Read the last saved timestamp
	if savedTime, err := os.ReadFile(timestampFile); err == nil {
		savedTimeStr := string(savedTime)
		if savedTimeStr == apkDbTime.Format(time.RFC3339) {
	...
	if err := os.WriteFile(timestampFile, []byte(apkDbTime.Format(time.RFC3339)), 0644); err != nil {
		logger.Warn(api.Tf("failed to save APK database timestamp: %v\n", err))
	}
...
func (d *PreloadDaemon) getFoldersToPreload() ([]string, error) {
	...
	if err != nil {
		logger.Error(api.Tf("failed to read category files: %v\n", err))
...
func (d *PreloadDaemon) preloadFolder(folder string) error {
	...
	if err != nil {
		logger.Error(api.Tf("failed to generate app list for '%s': %v\n", folder, err))
	...
	if err := saveCachedList(config, list); err != nil {
		logger.Error(api.Tf("failed to save cached list for '%s': %v\n", folder, err))
	...
	if err := tc.SaveTimestamps(folder); err != nil {
		logger.Error(api.Tf("failed to save timestamps for '%s': %v\n", folder, err))
...
func (d *PreloadDaemon) RefreshAllCategories() error {
	folders, err := d.getFoldersToPreload()
	if err != nil {
		logger.Error(api.Tf("failed to get folders to preload: %v\n", err))
		return fmt.Errorf("failed to get folders to preload: %w", err)
	}

	for _, folder := range folders {
		if err := d.preloadFolder(folder); err != nil {
			logger.Error(api.Tf("failed to refresh category '%s': %v\n", folder, err))
...
func StartPreloadDaemon(directory string) (*PreloadDaemon, error) {
	...
	if err := daemon.Start(ctx); err != nil {
		logger.Error(api.Tf("failed to start preload daemon: %v\n", err))
