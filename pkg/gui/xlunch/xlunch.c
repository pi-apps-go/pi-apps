...
static int xlunch_initialized = 0;
static FILE* cgo_input_source = NULL;

int xlunch_init(int argc, char **argv) {
    if (xlunch_initialized) {
        return 0; // Already initialized
    }
    
    // Initialize xlunch with provided arguments
    init(argc, argv);
    
    xlunch_initialized = 1;
    return 0;
}

void xlunch_set_entries_from_string(const char* entries_data) {
    if (!entries_data) return;
    
    // Create a temporary file or use a memory stream
    // For simplicity, we'll create a temporary pipe
    int pipefd[2];
    if (pipe(pipefd) == -1) return;
    
    // Write data to pipe
    write(pipefd[1], entries_data, strlen(entries_data));
    close(pipefd[1]);
    
    // Set as input source
    cgo_input_source = fdopen(pipefd[0], "r");
    input_source = cgo_input_source;
}

void xlunch_set_config_option(const char* option, const char* value) {
    // This would be used for dynamic configuration
    // For now, configuration is handled through command line arguments
}
...
int xlunch_run_loop() {
    ...
    // This is extracted from the main function
...
void xlunch_cleanup() {
    if (cgo_input_source) {
        fclose(cgo_input_source);
        cgo_input_source = NULL;
    }
    cleanup();
    xlunch_initialized = 0;
}

// Keep the original main function as an alias for backwards compatibility
int xlunch_main(int argc, char **argv) {
